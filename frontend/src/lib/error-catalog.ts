...
export const ERROR_CATALOG: Record<string, ErrorInfo> = {
  ...
  "E-VALID-001": {
    message:
      "Output gol/incomplet — executia nu a livrat rezultat (HTML/fisier gol)",
    cause:
  ...
  "E-NET-002": {
    message:
      "Raspuns HTTP cu eroare (4xx/5xx) de la API — executia nu a livrat",
    cause:
  ...
  "E-NET-003": {
    ...
    fix: "Toate citirile de JSON de la API-ul Python trec prin `lib/json-response.ts` (`readJson`), care taie gunoiul dinaintea primului `{`/`[` si reia parsarea. Severitate `warn` = eroarea a fost RECUPERATA automat, utilizatorul n-a vazut nimic. Daca apare des, verifica functiile Python din `api/` care scriu antete dupa `end_headers()`.",
    severity: "warn",
    area: "network",
  },
  "E-RATE-001": {
  ...
  "E-TEST-001": {
    ...
    cause:
      "Generarea din modulul Teste s-a oprit: lantul AI n-a livrat, raspunsul a fost gol sau taiat, ori construirea testului a crapat.",
  ...
  "E-SCOL-001": {
    ...
    cause:
      "Generarea din modulul Scolare s-a oprit: lantul AI n-a livrat, raspunsul a fost gol sau taiat, ori construirea fisei a crapat. Nodul curricular cerut e in context.node.",
