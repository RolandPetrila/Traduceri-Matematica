...
- [ ] **F1 — Editor core + G1 (formatare)**: `useEditor` (config Mösslein) + toolbar desktop (ToggleGroup/Select/DropdownMenu) + **toolbar mobil (bottom Sheet)**. Foaia A4 responsive (fără padding fix desktop). Gate: G1 complet + mobil verificat la 390px (iframe-probe).
