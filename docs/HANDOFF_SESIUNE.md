...
- ➡️ **URMĂTORUL: R6** (search GLOBAL Ctrl+K peste toată aplicația) — **UI NOU → §17: mock + confirmarea Roland ÎNAINTE de cod.** Iconițe/UI: `lucide-react` + shadcn (`Command`/`Dialog`) DEJA în proiect (R-COST, nu adăuga pachete). Ctrl+K să nu intre în conflict cu Ctrl+F (find editor). Apoi R4 → §2. **RĂMAS pe R3+R5: eyeball Roland pe prod** (import `.docx` real + F8 pe telefon). ⚠️ Advisor: liniile ~310-450 din acest HANDOFF necitite integral (istoric); grep înainte de lucru care le atinge.
