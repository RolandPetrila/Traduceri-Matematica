...
- 🆕 **CERINȚE NOI de la Roland (2026-07-30 runda 2), stabilite prin AskUserQuestion — scrise în `PLAN_MASTER §1`:**
  ...
  - **OCR/DOCX (extinde R3+R4):** fișierele lui Roland `test5nr.naturale2025.docx` + `2.Unghiuri. Bisectoare.docx` (+ `test.multimi2.docx`) = test set R3. Dovada bug: `Downloads/*.pdf` au matematica DISPĂRUTĂ. Fidelitate: math+ordine ÎNTÂI (ferm), apoi vizual. R4 = și OCR imagine/scan pe mai multe tipuri.
- ➡️ **ORDINE (aleasă de Claude, Roland „alegi tu"): R3 → R5 → R6 → R4 → §2 securitate → §3/§4/§5/§6.** **Începe cu PASUL 3 = R3** (DOCX OMML→LaTeX, bug vizibil): modul pur `lib/omml-to-latex.ts` + fixture REAL din cele 3 `.docx` + integrare `editor-import.tsx` (înlocuiește `mammoth.extractRawText`, cere cititor zip `fflate`/`jszip`) + reuse `parseInlineToNodes` din `ocr-map.ts`. ⚠️ Advisor: liniile 277-422 din acest HANDOFF necitite integral (istoric); grep înainte de lucru care le atinge.
