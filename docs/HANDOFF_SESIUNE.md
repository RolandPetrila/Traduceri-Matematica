...
- ➡️ **URMĂTORUL PAS = PASUL 3 = R3 (DOCX OMML→LaTeX, §1 R3)** — bug vizibil în editor („Fie multimile ," fără mulțimi). Modul pur `lib/omml-to-latex.ts` + fixture REAL din `test.multimi2.docx` (20 OMML) + integrare `editor-import.tsx` (înlocuiește `mammoth.extractRawText`, cere cititor zip `fflate`/`jszip`) + reuse `parseInlineToNodes` din `ocr-map.ts`. ⚠️ Advisor: liniile 277-422 din acest HANDOFF necitite integral (istoric); grep înainte de lucru care le atinge.
