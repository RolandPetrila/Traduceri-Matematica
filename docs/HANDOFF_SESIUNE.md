...
- ➡️ **URMĂTORUL:** ✅ deploy grupat FĂCUT (v44). **C (modul Școlare) — PLAN SCRIS: `docs/PLAN_SCOLARE_2026-08-07.md` v1.1** (research LIVE reformă + explorare folder-sursă Carla + AskUserQuestion 3 decizii + 2× advisor). **Așteaptă confirmarea Roland + răspuns la 1 întrebare (F0 acum sau sesiune nouă).** NU s-a scris cod (R-PLAN). Decizii cheie: modul nativ React separat „Școlare 🌐" (AI/online, reutilizează `/api/proxy`+Teste); unitate „o materie odată"; pilot Gimnaziu Clasa 5 Matematică; liceu în skeleton marcat „în tranziție"; **acoperirea 100% = SKELETON complet (16 nivele) livrat în F0**, conținutul e multi-sesiune. **Descoperire: bug „7 regulamente" CONFIRMAT** — Gimnaziu 6/7/8 + Liceu 9-12 au regulament copiat byte-identic din Clasa 5 (nu au reguli proprii).
