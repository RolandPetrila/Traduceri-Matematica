...
- ➡️ **URMĂTORUL PAS = PASUL 2 = R1 (icon-rail Mosslein, §1 R1).** Cere: **advisor înainte** (capcana hidratare localStorage-in-useState → vezi [[finding_hydration_tab_and_deploy_verify_2026_07_26]]: init DEFAULT, citește persist+breakpoint în `useEffect`, ca `EditorMathMenu`), citit `Mosslein .../Sidebar.tsx`, mock deja aprobat, probă live pe TOATE taburile (inclusiv iframe Asistent/Planșe) la 390px+desktop, constrângere: taburi montate simultan cu `display:none` — NU rute Next. Înainte de R1: `git grep -n -iE "display:none|page\.tsx|iframe|montate|layout|TopBar" -- docs/HANDOFF_SESIUNE.md` (advisor: liniile 277-422 necitite).
