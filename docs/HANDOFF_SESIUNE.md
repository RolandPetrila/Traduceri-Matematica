...
- ➡️ **URMĂTORUL: R5** (mut butoanele F8 „RO/SK/EN/DE" în rândul de sus al `TiptapToolbar`, după grupul culori/clear ~L306-371; probă vizuală) → **grupez deploy R3+R5** → propun deploy lui Roland (bump `CACHE_VERSION`). Apoi R6 (Ctrl+K, mock §17) → R4 → §2. ⚠️ Advisor: liniile ~300-440 din acest HANDOFF necitite integral (istoric); grep înainte de lucru care le atinge.
