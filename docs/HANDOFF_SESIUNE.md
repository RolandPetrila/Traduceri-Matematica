...
- ➡️ **URMĂTORUL: R4** (OCR imagine/scan — provider pe DOVADĂ măsurată). Primul sub-pas OBLIGATORIU (R3-onestitate): confirmă la sursă (docs oficiale) dacă **Azure Document Intelligence** extrage formule ca LaTeX (`[PROBABIL]`, NEverificat) — dacă nu, comparația se reduce la Mistral vs Gemini. Set de test = DOAR imagini/scanuri (`.docx`-urile merg prin R3!). Raport `docs/OCR_COMPARATIE_*.md` + declară costul din cote free. Apoi **§2 securitate** (S1 `npm audit fix` — ⚠️ capcană `katex@0.16.11` pinuit pt `@tiptap/extension-mathematics@3.28.0`; S2 XSS `HistoryDetail.tsx:65` `document.write` nesanitizat, fix 1 linie). **RĂMAS: eyeball Roland pe prod** (R3 import `.docx`, R5 F8 sus, R6 Ctrl+K — pe telefon). ⚠️ Advisor: liniile ~330-470 din acest HANDOFF necitite integral (istoric); grep înainte de lucru care le atinge.
