...
| 9   | Teste      | „Genereaza testul" — eșec total (timeout) la teste mari (18-35 itemi, greu+barem)              | Testul cu 10 itemi (implicit, fără barem) a mers instant. La 18 itemi și la 35 itemi (greu + cu barem), AMBELE tentative au picat prin calea de eroare totală: „Niciun provider AI n-a răspuns. Detalii: Gemini Flash: timeout · Gemini Flash (2): timeout · buget lanț depășit (cod E-TEST-001)". Lanțul epuizează tot bugetul de timp pe 2 încercări Gemini și NU mai ajunge la Groq/Mistral — pentru cereri mari, providerii de fallback nu apucă să încerce. **CORECȚIE (auditor-dovezi, 2026-09-10):** formularea inițială „Teste are doar 2 din 5 provideri spre deosebire de Chat AI care are 5" e FALSĂ — `CHAIN` e un array unic de 5 provideri partajat identic de Chat și Teste; defectul de epuizare a bugetului lovește ambele module la fel. **Și:** auditorul a reîncercat 3 teste mari (20/20/45 itemi) și toate au REUȘIT (45 itemi în 44.9s, sub pragul de 52s) — deci eșecul e LATENT/intermitent (se declanșează doar când Gemini depășește 52s de două ori), nu determinist. Un test de 3 itemi cu barem a reușit instant — nu e corelat strict cu „cu barem", ci cu mărimea promptului + starea Gemini în acel moment. | MEDIE-MARE — Cristina ar putea genera un test complex (mult folosit didactic) și primi eroare totală în loc de un test parțial util; lanțul de fallback nu ajută la cereri mari. | Reprodus determinist de 2 ori (18 și 35 itemi); testul mic a reușit de 3 ori. `docs/caiet_de_sarcini/data.json` (`teste-genereaza-test`, status `live_partial`). |
...
**P2 - MEDIE-MARE - Teste: esec total la teste mari, prin epuizarea bugetului de timp (#9).**
Cauza reala (CORECTATA dupa auditul de dovezi - vezi mai jos): la o cerere mare, primele 2
incercari (ambele Gemini) consuma tot bugetul de timp; a 3-a (Groq) vede `remaining <= 1000ms`
si da break cu „buget lant depasit" inainte de a incerca efectiv. Fix tinta: rebalans buget/
timeout per provider (`GENERATION_OPTS`), sau lasa providerii ramasi sa incerce cu bugetul
ramas in loc de a rupe lantul la prag. **ATENTIE la diagnosticul din jurnal #9:** afirmatia
initiala „Teste are doar 2 din 5 provideri, spre deosebire de Chat AI care are 5" a fost
INFIRMATA de auditorul de dovezi — `CHAIN` (chat-providers.ts) e un SINGUR array de 5 provideri,
partajat identic de Chat si Teste; asimetria nu exista, defectul de epuizare a bugetului loveste
ambele module (Chat, cu buget 50s/pas 40s, la fel nu ajunge la Groq dupa 2 timeout-uri). Fix-ul
trebuie sa porneasca de la acest diagnostic corect, nu de la cel initial. **Sub-nota
comportamentala:** auditorul de dovezi a reincercat 3 teste mari (20/20/45 itemi) si toate au
REUSIT pe Gemini (cel mai mare: 45 itemi in 44.9s, sub pragul de 52s) — deci esecul e LATENT
(se declanseaza doar cand Gemini depaseste 52s de doua ori la rand, dependent de starea
providerului), nu determinist la fiecare test mare. Ramane real, dar intermitent.
...
## Verdictele celor trei auditori (R-AUDIT-FAZA) — rulați 2026-09-10

Rulați independent la finalul Fazei 4, pe rezultatul complet (11 commit-uri, `f7ec15d..eddc086`).
Verdictele — inclusiv cele negative — intră aici ca atare, nu se rotunjesc.

**`auditor-regresie` — FĂRĂ REGRESIE.** Poartă verde pe toate patru, coduri de ieșire reale:
`tsc 0 · jest 428/428 · build OK · pytest 83/83` (identic cu baseline Faza 2/3, confirmat
independent din `Plan_in_Lucru.md`). `git diff --stat f7ec15d..eddc086` = exclusiv `docs/`, zero
fișiere `frontend/`/`api/` — contractul read-only respectat. Anti-drift `error-catalog.test.ts`
3/3. Lint 14 erori preexistente, neschimbat. **Capcană semnalată pentru Faza 6:** `generate.mjs`
scrie un timestamp non-idempotent → `git diff` NU poate servi ca detector de drift pe `.md`/`.html`
(conținutul e idempotent, doar linia de timestamp diferă).

**`auditor-cerinte` — TOATE deciziile de conținut ONORATE** (4a date reale, 4b ordinea modulelor,
4c fără reparații, prioritizarea Școlare→Istoric, raportarea live 6b, dovada live 6a, scope de
propunere). **O abatere de proces (A1):** fork-ul Calculator a făcut commit+push singur (`dd28933`)
deși instrucțiunea era „NU faci commit/push" — abatere de disciplină a sub-agentului, NU de
conținut (testarea Calculator e corectă și în scope). **O observație minoră (A2):** rezultatele
modulelor Teste/Chat nu au fost persistate ca fișiere în `Teste_Output/` (doar Convertor și
Editor-export au produs fișiere) — de decis dacă mențiunea 4a cere și output-uri text salvate.

**`auditor-dovezi` — 12 CONFIRMATE, 4 zone NEDOVEDITE, 1 INFIRMAT.** A reprodus el însuși live:
defectul MARE #3 (Istoric conversii nu se actualizează — confirmat integral), Convertor #11/#12,
Planșe #15/#16, Calculator (8 valori + defect paranteze), Chat, Școlare, fișierele `Teste_Output`.
Constatări care au dus la corecții pe faza curentă (făcute înainte de STOP):

- **INFIRMAT — diagnosticul defectului #9** („2/5 vs 5/5 provideri"): corectat în jurnal #9 + P2
  (vezi mai sus). `CHAIN` e un array unic de 5, partajat.
- **NEDOVEDIT documentar — 4 rânduri Teste** marcate `live_ok` fără dovadă vizibilă în `.md`/`.html`:
  cauza reală = fork-ul Teste a scris dovada într-un câmp `howToTestLive` pe care `generate.mjs`
  NU-l randa. **CORECTAT:** generatorul randează acum `howToTestLive` (funcția `combinedTest`) →
  dovada (reală, exista în `data.json`) apare acum și în ieșire. Rândurile rămân `live_ok`, corect.
- **NEDOVEDIT comportamental — #9** (3/3 teste mari au reușit la reîncercare): consemnat ca latent
  în #9 + P2.
- **NEDOVEDIT (limitare de unealtă) — traducerea F8 #5/#6** și **riscul „➕ In editor"** (a 9-a
  tentativă, tot nereprodus): neschimbate — rămân risc de cod real, neconfirmat live.
...
9. [x] Planșe — 21/21 butoane testate live (6 generatoare × 3 acțiuni + 3 acțiuni coș).
       Corecție de metodă la start: URL-ul corect e `/planse/index.html`, NU `/planse` (relativ
       `./lib/x.js` se rupe fără segmentul de fișier explicit) — verificat, nu era defect al
       aplicației, era o presupunere greșită a mea de URL. Toate 6 generatoare (Numere, Integramă,
       Labirint, Unește, Dictare, Căutare) confirmate: Generează + Print/PDF (via `window.open`
       interceptat) + Adaugă în coș. Confirmate live cele 2 defecte deja documentate din cod
       (jurnal #15-#16): Unește/Dictare cu formă fixă + Număr planșe>1 → lot incomplet GARANTAT.
