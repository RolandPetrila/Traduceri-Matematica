...
| F1     | Gimnaziu Matematică — regulamente proprii Clasa 6/7/8 (vezi §8 bug) + generare validată             | M                      |
| F2     | Gimnaziu toate materiile (regulamente Roland, 7-11 materii/clasă)                                   | L                      |
| F3     | Primar (Clasa 0-4) — materii + domenii DP; schema dual-shape testată aici                           | M-L                    |
...
| 2026-08-07 | Plan v1.1 | ✅ revizuit | Advisor: skeleton-first (16 nivele) în F0 = acoperirea 100%; regulament = asset separat de skeleton; bug „7 regulamente" VERIFICAT+confirmat (copii Clasa 5); plafon prompt `/api/proxy` verificat (8192 output, input OK). **Așteaptă confirmarea Roland.** |
