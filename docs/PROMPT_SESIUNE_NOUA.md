...
· R3 fixture-uri REALE (test set, în C:\Users\ALIENWARE\Desktop\Cristina\Fisiere_Word\), VERIFICATE la sursă 2026-07-30 (`unzip -p "<f>.docx" word/document.xml | grep -c "m:oMath"`): test.multimi2.docx = 20 OMML / 0 imagini · test5nr.naturale2025.docx = 9 OMML / 0 imagini · „2.Unghiuri. Bisectoare.docx" = 6 OMML / 1 IMAGINE (word/media → importă și imaginea, ca ResizableImage, nu doar OMML). Dovada bug-ului: Downloads\test5nr.naturale2025.pdf (output actual al app-ului) are matematica DISPĂRUTĂ complet. (Re-verifică prezența celor 3 fișiere la start.)
...
· R5 (toolbar): PĂSTREAZĂ toolbar-ul cum e — mută DOAR butoanele de limbi (F8 „scris în RO SK EN DE", azi pe rând separat sub TiptapToolbar; butoane <button> simple cu textContent RO/SK/EN/DE) în rândul de SUS, după evidențiere + „Șterge formatarea" (TiptapToolbar.tsx, grupul culori/clear ~L306-371). Am RESPINS explicit redesign-ul în module-panel.
· R6 (search global): pe lângă search-ul existent din meniul Matematică (rămâne), adaugă UN search GLOBAL stil Ctrl+K peste TOATĂ aplicația (funcții editor + comută între module + acțiuni). R6 e UI nou → §17 mock înainte de cod. Iconițe/UI: lucide-react (^0.363.0) + shadcn (Command/Dialog) sunt DEJA în proiect — NU adăuga pachete (R-COST). Ctrl+K să nu intre în conflict cu Ctrl+F (find-ul editorului).

EXECUTĂ ÎN ACEASTĂ ORDINE (am zis „alegi tu"; ordinea aleasă, motivată în PLAN_MASTER §1):
PASUL 1. R3 (DOCX OMML→LaTeX) — bug VIZIBIL pe prod, prioritar. Modul pur frontend/src/lib/omml-to-latex.ts (unit-testabil fără DOM) + fixture real din word/document.xml + cititor zip (fflate/jszip, înlocuiește mammoth) + reuse parseInlineToNodes din ocr-map.ts. ⚠️ CAPCANĂ: parseInlineToNodes tratează $ ca delimitator ($$ înainte de $, latex-gol→literal) — dacă OMML→LaTeX emite $ literal în text sau \{ din <m:d>, reuse-ul mângle output-ul; emite LaTeX curat / escape $. Plus: importă și imaginile din word/media (2.Unghiuri are 1). Eyeball pe cele 3 .docx: import → formule la locul lor → export PDF comparat cu varianta ruptă (Downloads\*.pdf).
