...
- [x] **C6 — Rute clarificate (2026-08-03, doc-only — NU sunt duplicat):** `/editor` = tabul embedded (antet + buton „Tot ecranul") · `/editor-nou` = vederea FULL-SCREEN (aceeași componentă `EditorTiptap`), deschisă din acel buton. Intenționate ambele. Reparat comentariul STALE din `editor-nou/page.tsx` (spunea „preview în dezvoltare până la F6"; F6 gata). NU se unifică (ar rupe butonul „Tot ecranul" + invariantul R1 taburi-nu-rute).
- [x] **C7 — CAPCANĂ RESPECTATĂ (2026-08-03):** NU am atins fișierele VII: `figure_crop.py` (hot-path OCR + editat la R8.4), `html_builder.py`, `ocr_structured.py` (editat la S4), `math_protect.py`. **+ADĂUGAT (advisor):** `azure_layout.py` (editat S4) + `deepl_client.py` = ȘI ele pe căi live. Curățenia §4 a atins DOAR orfani cu 0 importatori dovediți. **+CORECȚIE plan:** `import re as _re` (marcat mort la C5) e VIU (`_sanitize_error`/S6) — neatins.
...
- [x] **M3 — Dark-mode = RESPINS DEFINITIV (Roland, 2026-08-04).** Aplicația rămâne pe tema VERDE „tablă + cretă" (R-THEME) în TOATE modulele. **NU se mai propune niciodată** ca item. (Decizie fermă — nu „deferat".)
...
## §6b. CERINȚE NOI ROLAND (2026-08-07 seara) — EXECUȚIE OBLIGATORIE (nu backlog opțional)

**Diferență față de §7:** aceste 4 item-uri NU sunt „listate, neimplementate" — Roland a cerut explicit ca fiecare sesiune viitoare să le vadă ca RESTANȚE de executat, nu ca opțiuni discreționare. Detaliu complet: `docs/PROMPT_SESIUNE_NOUA_2026-08-08.md`.

- [~] **A — Integramă multi-formă** (ÎN LUCRU, sesiune 2026-08-08 — Roland a ales „toate 3, pe rând": Zigzag → Cruce → Scară). Motorul solver (`countSolutions`/`canForcePropagate`/`evalOp`) e topologie-agnostic (verificat 2026-08-08: ia `structure.equations` ca date); geometria (`cellsFromRaw*`/`equationLayout*`/`GEOM[forma]`) e per formă — fiecare formă nouă cere propria geometrie + propriul round-trip independent + propriul selftest.
  - [x] **Zigzag** (commit vezi git log 2026-08-08): coloană vertebrală cu 1-2 coturi 90° (Ușor n=2 dirOut=[E,S] / Standard n=3 [E,S,S] / Greu n=4 [E,S,S,E]), reutilizează `buildRaw` neschimbat (motor abstract L/R/U/D). Geometrie nouă: `cellsFromRawZigzag`+`equationLayoutZigzag` (independente, dispatch prin `DIRV`/`DIR_ORDER`/`oppositeDir`), `armCells` generic (checkItem 2b nu mai hardcodează poziții „moară"), `data-forma/dif/rows/cols` pe grilă (parsePuzzleHtml robust, nu mai infera n din cols), `trackSizes` 14/8mm pt grid-template-rows/columns dinamic (zigzag), `signature` include forma (P4 dedup corect cross-formă), `buildOne` acceptă `forma:"aleator"` (rng.choice(FORME), ca la dictare.js). UI: selector „Formă" în `mountIntegrama` (`app.js`), listă `dictare.js`-style. **Gate:** selftest 6/6 combinații formă×dificultate × 24 seed-uri verde (`__SELFTEST_OK__===true` pe `selftest.html` local :8899) + **bug de POZIȚIE plantat manual** (swap free[0]/free[1] la U-arm) → toate seed-urile zigzag FAIL, revert → verde din nou (dovadă că round-trip-ul chiar verifică poziția, nu doar glifele). **Dovedit LIVE** (static :8899 + Chrome MCP): Greu, formă=Zigzag din UI → grilă randată corect (13 ecuații verificate manual în consolă, toate OK, traseul confirmă E-S-S-E), soluție toggle corectă, print-fit (17×13 celule ≈190×146mm, sub limita A4 utilă 273×186mm). NEDEPLOYAT.
  - [x] **Cruce** (commit vezi git log 2026-08-08): hub comun (4 brațe LINK: V/E/N/S) + `buildRaw` extins cu `forcedX1`/`forcedPerm` (opționale, aditive — al 2-lea lanț pornește din ACELAȘI X1, cu `perm` ales din operațiile NEFOLOSITE de primul lanț la L/R, ca hub-ul să aibă 4 operații distincte prin CONSTRUCȚIE, nu respingere). **Coliziune reală descoperită + rezolvată** (rundă advisor 2026-08-08): la nH>=2 ȘI nV>=2 simultan, brațul liber al morii orizontale 2 și cel al morii verticale 2 ajung pe ACEEAȘI celulă (colț comun la pas 4) — mecanism WIDE_GAP (pas dublu) rezolvă coliziunea de DATE dar la eyeball arată ecuația verticală RUPTĂ de goluri → **respins după probă live**, înlocuit cu bănzi de dificultate collision-free (Greu = nH:1,nV:3, un singur braț crește per dificultate) + invariant asertat în selftest. Gate: 9/9 formă×dificultate ×24 seed-uri + bug de poziție plantat (hub N-block mutat pe coloană greșită) → FAIL, revert → verde. Dovedit LIVE (static:8899 + Chrome MCP): Greu prin UI, 10 ecuații verificate manual OK, render curat fără goluri vizuale (confirmat vizual după respingerea WIDE_GAP). NEDEPLOYAT.
  - [ ] **Scară** — 2 coloane vertebrale + trepte, graf CU cicluri → tehnica „set ascuns ACICLIC" de la numere.js (nu euristica simplă de la moară/zigzag).
- [ ] **B — Extinde varietatea la celelalte generatoare Planșe.** Ordine ieftin→scump: unește (catalog forme 7→mai multe) → dictare (catalog 17→mai multe) → căutare (teme 5→mai multe) → numere (grile 4×4/5×5, nu doar 3×3) → labirint (forme/ieșiri multiple, risc mai mare pe invarianții arbore-perfect+BFS, ultimul).
- [ ] **C — Modul „Școlare 🌐" full-curriculum RO** (grădiniță→liceu). Decizie de arhitectură REZOLVATĂ (risc copyright discutat cu Roland, 2026-08-07): sursă de CONȚINUT = programa oficială (rocnee.eu, document public); manualele MEN (`manuale.edu.ro`, legale/gratuite) = DOAR referință de aliniere, niciodată stocate/redistribuite; conținut nou = original AI aliniat curricular (modelul `project_curriculum_audit_2026_07_28`); PDF-uri de referință DOAR local gitignored, NICIODATĂ committed. Cerință fermă: acoperire 100% a claselor/categoriilor (pilotul e la alegerea sesiunii, ținta finală nu se negociază). **Necesită `PLAN_SCOLARE_[data].md` (R-PLAN) + AskUserQuestion propriu ÎNAINTE de cod** — NU începe direct.
- [ ] **D — Bug randare `\lim` (identificat, NU mai e „nereprodus").** Repro confirmat de Roland (2026-08-08, `Screenshot (260).png`): în `99_Roland_Work/Teste_Output/limite_matematica.pdf`, notația `lim_{x→∞}` apare cu subscriptul `x→∞` ca indice inline lipit de „lim" (stil TEXT/inline KaTeX), în loc de stivuit CENTRAT sub „lim" (stil DISPLAY, ca în original `limite_matematica.jpeg`). Conținutul/cifrele sunt corecte (consistent cu scorul math 10/10 din `finding_ocr_test_scorecard_2026_07_31`) — e strict o problemă de STIL de randare KaTeX (inline vs displaystyle/`\operatorname*`), nu o pierdere de conținut OCR. Candidat fix ieftin: forțează `\displaystyle` sau `\operatorname*{lim}` pe LaTeX-ul generat pt `\lim`, verifică nu regresează alte formule cu limite inline (text curent).
