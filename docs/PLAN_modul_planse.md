...
| ---------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ----------------- |
| **F0 Schelet (NATIV)**             | Convenția iframe-module (§16.3) + tab „Planșe" deschide shell-ul gol                      | §16.3 (shell nativ) — **fallback manual §4**; + `public/planse/{index.html,app.js,style.css}` | **S-M**           |
| **F1 Primul generator (Labirint)** | Labirint cap-coadă: formular → generează → preview → Print/PDF. Validează tot pipeline-ul | `lib/prng.js`, `generators/labirint.js`, selftest+oracle, `render.js`                         | **M** (1 ses.)    |
