...
- D52: **Scope Școlare REVIZUIT (Roland, 2026-08-08, după F3 deployat):** acoperire cu TOATE materiile/domeniile DOAR pt **Grădiniță + Primar 0-4**. Primar 0-4 toate materiile = DEJA FĂCUT (F3). Deci **următorul = Grădiniță (F4), toate domeniile**. **Gimnaziu + Liceu = DOAR Matematică deocamdată** (gimnaziu mate = F1 gata; nu se adaugă non-mate la gimnaziu/liceu acum → vechiul „F2 gimnaziu toate materiile" e AMÂNAT). **VIITOR (la cererea explicită a lui Roland): extindere Cl.5-12 cu TOATE materiile non-mate** — documentat + gata de executat direct (skeleton-ul le are deja ca noduri; lipsește doar conținutul/regulamentele). Coada C nouă: **F4 (Grădiniță) → [la cerere: F2 gimnaziu non-mate + F5 liceu]**.
- D53: **Loguri diagnostic: citire protejată + fără nume (Roland, 2026-09-23, AskUserQuestion ×3 — închide M4 din auditul 2026-08-08, redescoperit de auditul GPT).** `GET /api/logs` cere header `x-diag-token` = env `TRADUCERI_DIAG_TOKEN` (fail-closed); `/diagnostics` cere codul o dată (localStorage); POST/telemetria rămâne publică. Numele documentelor/fișierelor NU intră în loguri (sursă + server: `frontend/src/lib/log-redact.ts`). Retenția logurilor rămâne **MANUALĂ, intenționat** (istoricul vechi servește R-DIAG-AUTO; tabela ~1 MB). Respinse: redactare fără token, risc acceptat, retenție automată 30 zile, branch protection (solo + auto-push). Fragmentele de CONȚINUT din loguri (traducere/dictare/find) = **SE PĂSTREAZĂ** (Roland, 2026-09-23 — mențiunea 1c cere conținut pt reproducere; acum nepublice). Token-ul procesat în master de Claude, la cererea lui Roland. Commit-uri `da54d84`, `2861bf8`, `69b3ada`.
